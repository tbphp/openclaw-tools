---
name: surge-control
description: 管理 Surge 代理客户端。当用户提到 Surge、代理、节点切换、策略组、VPN 线路、切换到某个国家/地区等关键词时使用此技能。
...
## 核心原则
- **Shell 安全**：在执行 curl 命令时，所有的 URL 必须使用双引号包裹，以防止 Zsh/Bash 将 `?`、`&` 等字符解析为通配符。
- 每次涉及策略组/节点匹配时，都先请求：
  `GET /v1/policy_groups`
- 永远用 API 返回的真实组名/节点名执行切换。
- 不写死策略组和节点名。
...
### 2. 切换策略组节点
1. 在第 1 步返回的 JSON 中匹配用户意图（如 ai -> AI，香港 -> 🇭🇰 香港）。
2. 调用切换：
```bash
...
- **禁止**直接把用户原始文本当作 `group_name` / `policy`。
- **禁止**跳过查询步骤直接执行切换操作。
